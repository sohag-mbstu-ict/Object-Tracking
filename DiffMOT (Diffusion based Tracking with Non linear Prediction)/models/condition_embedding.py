...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, x, class_token=False):
        ...
        tensor1 = pos_y[:, :, 0::2].sin()  # Shape: [16, 8, 3]
        tensor2 = pos_y[:, :, 1::2].cos()  # Shape: [16, 8, 2]
        # tensor2 = torch.cat((tensor2, torch.zeros(tensor2.shape[0], tensor2.shape[1], 1)), dim=2)

        pos_y = torch.stack((tensor1, tensor2), dim=3).flatten(2)
        return pos_y
...
class History_motion_embedding(nn.Module):
    ...
    def forward(self, x):
        ...
        # linear_layer = nn.Linear(1280, 8)
        # x = linear_layer(x)
        # print("inside condition_embedding.py : ",x.shape)
        # a = self.proj(x)
        # a
        ...
        for i in range(self.cascade_num):
            ...
            # print("Inside condition_embedding en_out : ",en_out)
    ...
    #     # Assuming the height (H) and width (W) of the image are known
    #     H, W,C = 720, 1280,3  # Example dimensions (height and width)
    #     C = 3  # Number of channels (e.g., RGB)

    #     # Reshape the tensor to the shape [N, C, H, W]
    #     # x = x.view( H, W, C).permute( 3, 1, 2)  # Reshape and permute to get [N, C, H, W]
    #     x = x.permute(2, 0, 1)

    #     # If batch size N is 1 (single image), add a batch dimension
    #     # x = x.unsqueeze(0)  # Shape: [1, C, H, W]

    #     # Convert to float if necessary (common for neural network inputs)
    #     x = x.float()
    #     q_patch = x#self.proj(x).permute(1, 0, 2)
    #     pos = self.pose_encoding(q_patch).transpose(0, 1)
    #     n, b, d = q_patch.shape
