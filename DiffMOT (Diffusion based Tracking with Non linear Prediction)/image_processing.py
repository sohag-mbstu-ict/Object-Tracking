...
class image_processing_techniques:
    ...
    def make_video_from_tracker(self,fg_result,output,make_video):

        c=0
        for bbox in output:
            deque_bbox = bbox.tlbr
            list_of_arrays = list(deque_bbox)
            box = [arr.tolist() for arr in list_of_arrays]
            box = deque_bbox#box[0]
            # print("box : ",box)
            xmin, ymin,xmax,ymax=int(box[0]), int(box[1]), int(box[2]), +int(box[3])
            id_name=bbox.track_id
