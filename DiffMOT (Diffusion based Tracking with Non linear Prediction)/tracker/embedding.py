...
class EmbeddingComputer:
    def __init__(self, config, dataset, test_dataset, grid_off, max_batch=1024):
        # ReID Model
        self.model=None
        # model_state_dict= torch.load("D:/Diffusion_based_MOT/sports_sbs_S50.pth",map_location=torch.device('cpu'))
        # self.model_state_dict.load_state_dict({k.replace('module.', ''): v for k, v in self.checkpoint['ddpm'].items()})
    ...
    def compute_embedding(self, img, bbox, tag):
        ...
        if self.grid_off:
            # Basic embeddings
            h, w = img.shape[:2]
            results = torch.round(bbox).to(torch.int32)
            # print(results)
    ...
    def initialize_model(self):
        ...
        if self.dataset == "mot17":
            ...
            # path = "D:/Diffusion_based_MOT/sports_sbs_S50.pth"
            # path = "C:/Users/Acer/Downloads/dance_sbs_S50.pth"
            path = "C:/Users/Acer/Downloads/mot20_sbs_S50.pth"
        else:
        ...
        # model.half()
        self.model = model
        # print("Inside embedding.py  model : ",model)
        return self.model

    def _get_general_model(self,model_path):
